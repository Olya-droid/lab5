...
public class Add {
     ...
     */
    public static void add(ConsoleSourceReader reader, Collection c) throws NullPointerException, IOException {
     ...
     * @param route объект
     */
    protected static void addName (ConsoleSourceReader reader, Collection c, Route route){
     ...
     * @param route объект
     */
    protected static void addCoordinates (ConsoleSourceReader reader, Collection c, Route route){
        ...
        int coordinateX;
        try { coordinateX = Checker.intChecker(s);}
     ...
     * @param route объект
     */
    protected static void addFrom(ConsoleSourceReader reader, Collection c, Route route){
        System.out.print("\n" + "Откуда? (from)" + "\n" + "long x = ");
        ...
        long locationFromX;
       try{ locationFromX = Checker.longChecker(s);}
       catch (NullPointerException e) {
        ...
        try {locationFromY = Checker.doubleChecker(reader.getLine());}
        catch (NullPointerException e) {
     ...
     * @param route объект
     */
    protected static void  addTo (ConsoleSourceReader reader, Collection c, Route route){
        System.out.print( "\n" +"Куда? (to)" + "\n" + "long x = ");
        String s = reader.getLine();
        if (  s.equals("") || s == null) {
            System.out.println("Введена пустая строка. Не надо так.");
        ...
        long locationToX;
        try { locationToX = Checker.longChecker(s);}
        catch (NullPointerException e) {
        ...
        System.out.print("double y = ");
        double locationToY;
        try {locationToY = Checker.doubleChecker(reader.getLine());}
        catch (NullPointerException e) {
        ...
        if ( locationToName.equals("") || locationToName == null) {
            System.out.println("Пустая строка ни к чему не приведёт. Пока ты смотришь в пустую строку, пустая строка смотрит в тебя...");
            return;
        }
     ...
     * @param route объект
     */
    protected static void addDist(ConsoleSourceReader reader, Collection c, Route route){
        ...
        float distance;
        try { distance = Checker.floatChecker(s); }
        catch (NullPointerException e) {
